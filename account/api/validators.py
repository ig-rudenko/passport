...
class RequestCodeValidator:
    ...
    def validate(self) -> bool:
        ...
        if getattr(self._request.user, "telegram_id", None) is None:
            raise AuthenticationFailed(
                {"detail": "Для данного пользователя не был указан telegram ID"}
            )
        ...
        if not self._request.user.temp_codes.filter(exp__gt=timezone.now()).count():
            ...
            notifier = settings.DEFAULT_NOTIFIER_CLASS(
                type_="telegram", user=self._request.user
            )
            try:
                notifier.notify(
            ...
            raise APIException(
                detail={"error": "Подтвердите код, отправленный на ваш телеграм/sms"},
                code=400,
            )
    ...
    def _validate_code(code: str) -> bool:
        try:
            ...
            raise AuthenticationFailed({"detail": e.args[0]})
